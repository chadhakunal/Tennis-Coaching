...
INPUT_FILE_1 = "Pose_Service/Input/8.avi"
INPUT_FILE_2 = "Pose_Service/Input/9.avi"
...
skel3DArray = getPose(frameArray_1)
skel3DArray2 = getPose(frameArray_2)

skel_plot = np.zeros((480, 640, 3, max(frameArray_1.shape[3],frameArray_2.shape[3])), dtype=np.uint8)

for i in range(min(frameArray_1.shape[3],frameArray_2.shape[3])):
    norm_skel3d1 = torch.from_numpy(skel3DArray[i])
    norm_skel3d2 = torch.from_numpy(skel3DArray2[i])
    fig = plt.figure(1)
    plt_3d: Axes3D = fig.add_subplot(1, 1, 1, projection='3d')
    plot_skeleton_on_axes3d(norm_skel3d1, CanonicalSkeletonDesc, plt_3d, invert=True)
    plot_skeleton_on_axes3d(norm_skel3d2, CanonicalSkeletonDesc, plt_3d, invert=True, colors=("green", "orange", "yellow", "purple"))
    # plt.show()
...
for i in range(frameArray_1.shape[3],frameArray_2.shape[3]):
    norm_skel3d2 = torch.from_numpy(skel3DArray2[i])
    fig = plt.figure(1)
    plt_3d: Axes3D = fig.add_subplot(1, 1, 1, projection='3d')
    plot_skeleton_on_axes3d(norm_skel3d2, CanonicalSkeletonDesc, plt_3d, invert=True, colors=("green", "orange", "yellow", "purple"))
...
for i in range(frameArray_2.shape[3],frameArray_1.shape[3]):
    norm_skel3d1 = torch.from_numpy(skel3DArray[i])
    fig = plt.figure(1)
    plt_3d: Axes3D = fig.add_subplot(1, 1, 1, projection='3d')
    plot_skeleton_on_axes3d(norm_skel3d1, CanonicalSkeletonDesc, plt_3d, invert=True)
...
outskel3D = cv2.VideoWriter('./skel_compare_8_9.avi', cv2.VideoWriter_fourcc(*'DIVX'), fps_1, (skel_plot.shape[1],skel_plot.shape[0]))
